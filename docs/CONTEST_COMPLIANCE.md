...
- **2026-08-08 change: the static PNG fallback was removed and the live vector map is now the only map surface.** `mapImageResourcePath` was deleted from `map_config.json`, and `PixelRoadApp` no longer loads a map texture. This was an explicit product decision and it **reopens** the offline-demo gate recorded below.
  ...
  - **2026-08-22 follow-up: the release gate was inverted.** `allowLiveVectorMapInRelease` and the opt-in `PIXELROAD_LIVE_VECTOR_MAP` symbol were removed, so every build ships the map by default. The offline-review flavor is now an explicit opt-out (`PIXELROAD_OFFLINE_REVIEW`), and only that flavor compiles out the requester and drops `INTERNET`.
  - This resolves the map-less release APK, but it does **not** resolve the offline-judging question: an offline-review APK still has no map because the static fallback is gone. Re-introducing an offline map source remains open.
